...
class DirectRefferal extends StatefulWidget {
  const DirectRefferal({super.key, required this.username});
  final String username;

  @override
  State<DirectRefferal> createState() => _DirectRefferalState();
}

class _DirectRefferalState extends State<DirectRefferal> {
  late List<dynamic> _results = [];
  

  Future<dynamic> getData(String username, String key) async {
    http.Response response = await http.get(Uri.parse(
      'https://evencross.online/dashboard/userapi/direct_referrals/$username/$key',
    ));
    if (response.statusCode == 200) {
      Map<String, dynamic> decodedData = json.decode(response.body);
      final status = decodedData['status'];
      if (status == 'success') {
        List<dynamic> data = decodedData["direct_referrals"];
        // print(data[0]["id"]);
        // print(data.length);
      }

      setState(() {
        _results = decodedData["direct_referrals"];
